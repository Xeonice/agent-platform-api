...
function hintFor(e: Error, target: Target, viaProxy: boolean): string {
  ...
  // ⛔ **超时不给「去配代理」这种下一步**（2026-09-09 用户质疑后实测补的）。
  //    同一台机器、同一分钟内连 api.openai.com 的 TLS 握手在 **2.3s / 2.5s / 10.2s**
  //    之间跳，而同一轮里"可达"的两条是 2430ms 与 3263ms —— 预算只比最慢的成功样本
  //    高 7%，判定实际上成了掷硬币。此时说「无法连接……填 HTTPS_PROXY」是**两重错**：
  //    ① 把「慢」说成「连不上」；② 给出的下一步（配代理）解决不了慢，用户照做之后
  //    问题还在，他会认为这项检查不准 —— 「说错下一步比不说更贵」，同 loopback 那条。
  //
