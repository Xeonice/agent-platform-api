...
export class ClaudeCodeAdapter implements RuntimeAdapter {
  ...
  async beginAuth(method: RuntimeAuthMethod, ctx: AuthSessionContext): Promise<AuthChallenge> {
    ...
    return {
      ...
      // ⛔ **这里不能有 Markdown 标记。** `AuthChallengeSchema.instructions` 是纯 `z.string()`，
      //    前端按纯文本渲染 —— 原文里的 `**…**` 会**原样带着星号上屏**。
      //    文字本身保留，只把标记去掉（强调靠断句，不靠符号）。
      instructions:
        '在浏览器打开链接完成登录。多数情况下你不需要再做别的：' +
        '页面显示「可以关闭此窗口」就说明授权已经自动送回来了，这里会自己变成已配置。' +
        '只有当页面显示了一串授权码时（浏览器和平台不在同一台机器上才会这样），' +
        '才需要把它粘到下面提交。',
    };
  }
