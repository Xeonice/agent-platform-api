...
export interface CredentialFacade {
  /**
   * Resolve + materialize the effective git credential for `(kind, host)` into an
   * opaque handle. `forKind` selects the credential → `host ∈ allowedHosts` is
   * enforced (I-CRD-8) → `credential/infrastructure` decrypts and materializes.
